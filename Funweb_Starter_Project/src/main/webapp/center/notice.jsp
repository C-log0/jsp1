...
<%-- JSTL의 형식(Formatting) 을 지정하기 위한 라이브러리 : fmt --%>
...
<%-- 						<td><fmt:formatDate value="${board.date }" type="time"/> </td> <!-- yyyy. MM. dd --> --%>
<%-- 						<td><fmt:formatDate value="${board.date }" type="both"/> </td> <!-- yyyy. MM. dd --> --%>
						<td><fmt:formatDate value="${board.date }" pattern="yy-MM-dd HH:mm"/> </td> <!-- yyyy. MM. dd -->
			...
			<div id="table_search">
				<!--  -->
				<%-- JSTL & EL 활용 --%>
				<c:if test="${not empty sessionScope.sId and sesseionScope.sId eq 'admin'}">
				</c:if>
...
//				System.out.println("전체 페이지 수 : " + maxPage);
				...
				//=========================================================
				//jstl과 el 을 통해 값에 접근하기 위해 pageContext 객체에 저장
