...
List<FreeBoardDTO> FreeBoardList = dao.selectFreeBoardList(startRow, listLimit, keyword);
...
<head>
<meta charset="UTF-8">
<title>center/free_board.jsp</title>
		...
		<article>
			...
			<table id="notice">
				...
				<%for(FreeBoardDTO freeboard : FreeBoardList) { %>
					<tr onclick="location.href='free_content.jsp?idx=<%=freeboard.getIdx() %>&pageNum=<%=pageNum%>'">
						<td><%=freeboard.getIdx() %></td>
						<td class="left"><%=freeboard.getSubject() %></td>
			...
			<div id="table_search">
				<form action="free.jsp" method="get">
...
// 				System.out.println("총 게시물 수 : " + listCount);
				...
				<%if(pageNum > 1) { %>
					<a href="free.jsp?pageNum=<%=pageNum - 1%>">Prev</a>
				<%} else { %>
				...
				<%if(pageNum < maxPage) { %>
					<a href="free.jsp?pageNum=<%=pageNum + 1%>">Next</a>
				<%} else { %>
