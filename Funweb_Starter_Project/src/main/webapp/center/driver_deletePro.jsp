...
// System.out.println(idx + ", " + pageNum + "pass");
...
// 삭제 결과 판별
// 성공 시 상세 조회 페이지(driver.jsp)로 이동하고// (=> 파라미터로 페이지번호 전달 필요)
// 아니면(실패 시) 자바스크립트로 "글 삭제 실패!" 출력 후 이전페이지로 돌아가기

		if(deleteCount>0){
			//삭제 성공 시 DB 에 있는 파일명과 일치하는 업로드 경로 상의 실제 파일을 삭제 후
			//글 목록 페이지(driver.jsp)로 이동하고(=> 파라미터로 페이지번호 전달 핖ㄹ요)
			//아니면(실패 시)자바스크립트로"글 삭제 실패!" 출력 후 이전 페이지로 돌아가기
		//------------------------------파일 삭제-------------------------------
