...
public class MyMessageDigest {
	...
	// 해싱(= 단방향 암호화 : 복호화가 안되므로)
	// 작업을 수행할 hashing() 메서드 정의
	// => 파라미터 : 평문 문자열    리턴타입 : String(암호문)
	public String hashing(String strPlainText) {
		String strCiperText = ""; //암호화 결과 문자열(=암호문)를 저장할 변수
		
		try {
			//java.security.MessageDigest 클래스의 static 메서드
			// getInstance() 메서드를 호출하여  MessageDigest 객체 리턴받기
			// => 파라미터 : 암호화 알고리즘명(=해시함수명)전달
