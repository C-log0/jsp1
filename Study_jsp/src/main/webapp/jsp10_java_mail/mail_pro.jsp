...
String receiver = "hz0123hz@gmail.com";
...
// =>  만약, TLS 버전 관련 문제 발생(could not convert soket to TLS)시


// 3. 메일 서버 인증 정보를 생성하는 사용자 정의 클래스 인스턴스 생성
GoogleMailAuthenticator authenticator = new GoogleMailAuthenticator();

// 4. 자바 메일에서 메일 전송을 수행하는 작업의 기본 단위를 javax.mail.Session 객체로 관리하므로
...
// -> 단, 상용 메일 서버(구글, 네이버 등)의 스팸 메일 정책으로 인해 발신자 주소 변경 불가
// => 파라미터 : 발신자 주소, 발신자 이름
Address senderAddress = new InternetAddress(sender, name);
